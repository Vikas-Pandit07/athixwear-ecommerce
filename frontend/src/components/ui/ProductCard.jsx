...
const ProductCard = ({
  ...
  const [isInCart, setIsInCart] = useState(false); // NEW: Track cart status
  const navigate = useNavigate();

  // FIX 1: Check if product is already in cart on mount
  useEffect(() => {
    checkIfInCart();
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.items) {
          ...
          const productInCart = data.items.some(item => item.productId === productId);
  ...
  const handleAddToCart = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // FIX: Update local state based on backend response
        setIsInCart(true);
        ...
        // Global event to update all components
  ...
  const calculateDiscount = () => {
    if (originalPrice && price) {
      const orig = typeof originalPrice === 'string' ? 
        parseFloat(originalPrice.replace('₹', '').replace(/,/g, '')) : originalPrice;
      const curr = typeof price === 'string' ? 
        parseFloat(price.replace('₹', '').replace(/,/g, '')) : price;
      if (orig > curr) {
  ...
  return (
    ...
    >
      ...
      <div className="product-info">
        ...
        {/* FIXED: Show View Cart only if product is actually in cart */}
        {isInCart ? (
          <button
